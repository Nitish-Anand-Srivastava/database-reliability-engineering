#!/usr/bin/env python3
"""Aurora PostgreSQL Observability Report snapshot and HTML report runner."""
...
DEFAULT_CONFIG = ROOT_DIR / "config" / "or_report.env"
DEFAULT_SQL_INSTALL = ROOT_DIR / "sql" / "00_install_or_repository.sql"
...
def configuration_review_rows(config: Dict[str, str], end_snapshot_id: int) -> List[Dict[str, str]]:
    rows = query_csv(
        ...
        WITH settings AS (
          ...
          FROM dbre_or.settings_snapshot
...
def build_report(config: Dict[str, str], start_id: int, end_id: int, output_path: Optional[Path] = None) -> Path:
    summary_rows = query_csv(
        ...
        f"""
        WITH start_snap AS (
          SELECT * FROM dbre_or.snapshots WHERE snapshot_id = {start_id}
        ),
        end_snap AS (
          SELECT * FROM dbre_or.snapshots WHERE snapshot_id = {end_id}
        )
        SELECT
    ...
    database_deltas = query_csv(
        ...
        f"""
        WITH start_stats AS (
          SELECT * FROM dbre_or.database_stats WHERE snapshot_id = {start_id}
        ),
        end_stats AS (
          SELECT * FROM dbre_or.database_stats WHERE snapshot_id = {end_id}
        )
        SELECT
          end_stats.datname,
    ...
    start_waits = query_csv(
        config,
        f"SELECT state, wait_event_type, wait_event, sessions, active_sessions, longest_query_age_seconds, longest_xact_age_seconds FROM dbre_or.activity_snapshot WHERE snapshot_id = {start_id} ORDER BY sessions DESC, active_sessions DESC"
    )
    end_waits = query_csv(
        config,
        f"SELECT state, wait_event_type, wait_event, sessions, active_sessions, longest_query_age_seconds, longest_xact_age_seconds FROM dbre_or.activity_snapshot WHERE snapshot_id = {end_id} ORDER BY sessions DESC, active_sessions DESC"
    )
    ...
    top_sql = query_csv(
        ...
        f"""
        WITH start_sql AS (
          SELECT * FROM dbre_or.statement_stats WHERE snapshot_id = {start_id}
        ),
        end_sql AS (
          SELECT * FROM dbre_or.statement_stats WHERE snapshot_id = {end_id}
        ),
        delta AS (
          SELECT
            end_sql.dbid,
    ...
    table_deltas = query_csv(
        ...
        f"""
        WITH start_tab AS (
          SELECT * FROM dbre_or.table_stats WHERE snapshot_id = {start_id}
        ),
        end_tab AS (
          SELECT * FROM dbre_or.table_stats WHERE snapshot_id = {end_id}
        )
        SELECT
          end_tab.schemaname,
          end_tab.relname,
    ...
    index_review = query_csv(
        ...
        f"""
        WITH start_idx AS (
          SELECT * FROM dbre_or.index_stats WHERE snapshot_id = {start_id}
        ),
        end_idx AS (
          SELECT * FROM dbre_or.index_stats WHERE snapshot_id = {end_id}
        )
        SELECT
          end_idx.schemaname,
          end_idx.relname,
    ...
    replication = query_csv(
        config,
        f"SELECT application_name, client_addr, state, sync_state, write_lag, flush_lag, replay_lag FROM dbre_or.replication_stats WHERE snapshot_id = {end_id} ORDER BY application_name"
    )
    slots = query_csv(
        config,
        f"SELECT slot_name, slot_type, active, wal_status, pg_size_pretty(safe_wal_size) AS safe_wal_size, inactive_since FROM dbre_or.replication_slots WHERE snapshot_id = {end_id} ORDER BY active ASC, slot_name"
    )
    ...
    if output_path is None:
        ...
        output_path = output_dir / f"or_{start_id}_{end_id}_{suffix}.html"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Aurora PostgreSQL OR-style report runner")
...
def main() -> int:
    ...
    if args.command == "install-sql":
        ...
        print("Installed dbre_or SQL repository.")
        return 0
