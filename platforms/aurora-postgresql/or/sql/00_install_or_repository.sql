...
CREATE SCHEMA IF NOT EXISTS dbre_or;

CREATE TABLE IF NOT EXISTS dbre_or.snapshots (
...
CREATE TABLE IF NOT EXISTS dbre_or.settings_snapshot (
  snapshot_id bigint NOT NULL REFERENCES dbre_or.snapshots(snapshot_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dbre_or.activity_snapshot (
  snapshot_id bigint NOT NULL REFERENCES dbre_or.snapshots(snapshot_id) ON DELETE CASCADE,
  state text,
...
CREATE TABLE IF NOT EXISTS dbre_or.session_snapshot (
  snapshot_id bigint NOT NULL REFERENCES dbre_or.snapshots(snapshot_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dbre_or.database_stats (
  snapshot_id bigint NOT NULL REFERENCES dbre_or.snapshots(snapshot_id) ON DELETE CASCADE,
  datid oid,
  datname name,
  numbackends integer,
...
CREATE TABLE IF NOT EXISTS dbre_or.bgwriter_stats (
  snapshot_id bigint PRIMARY KEY REFERENCES dbre_or.snapshots(snapshot_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dbre_or.wal_stats (
  snapshot_id bigint PRIMARY KEY REFERENCES dbre_or.snapshots(snapshot_id) ON DELETE CASCADE,
  wal_records bigint,
  wal_fpi bigint,
  wal_bytes numeric,
...
CREATE TABLE IF NOT EXISTS dbre_or.table_stats (
  snapshot_id bigint NOT NULL REFERENCES dbre_or.snapshots(snapshot_id) ON DELETE CASCADE,
  relid oid NOT NULL,
...
CREATE TABLE IF NOT EXISTS dbre_or.index_stats (
  snapshot_id bigint NOT NULL REFERENCES dbre_or.snapshots(snapshot_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dbre_or.statement_stats (
  snapshot_id bigint NOT NULL REFERENCES dbre_or.snapshots(snapshot_id) ON DELETE CASCADE,
  dbid oid NOT NULL,
  userid oid,
...
CREATE TABLE IF NOT EXISTS dbre_or.replication_stats (
  snapshot_id bigint NOT NULL REFERENCES dbre_or.snapshots(snapshot_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dbre_or.replication_slots (
  snapshot_id bigint NOT NULL REFERENCES dbre_or.snapshots(snapshot_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS dbre_or.io_stats (
  snapshot_id bigint NOT NULL REFERENCES dbre_or.snapshots(snapshot_id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_dbre_or_snapshots_type_time
  ON dbre_or.snapshots (snapshot_type, captured_at DESC);

CREATE INDEX IF NOT EXISTS idx_dbre_or_table_stats_snapshot
  ON dbre_or.table_stats (snapshot_id, relid);

CREATE INDEX IF NOT EXISTS idx_dbre_or_index_stats_snapshot
  ON dbre_or.index_stats (snapshot_id, indexrelid);

CREATE INDEX IF NOT EXISTS idx_dbre_or_statement_stats_snapshot
  ON dbre_or.statement_stats (snapshot_id, dbid, queryid);

CREATE OR REPLACE FUNCTION dbre_or.rel_fillfactor(rel_oid oid)
RETURNS integer
LANGUAGE sql
STABLE
AS $$
...
CREATE OR REPLACE FUNCTION dbre_or.take_snapshot(
...
DECLARE
  ...
  v_has_pss_blk_read_time boolean;
  v_has_pss_blk_write_time boolean;
  v_has_pss_wal_bytes boolean;
BEGIN
  IF NOT pg_try_advisory_lock(v_lock_key) THEN
    RAISE EXCEPTION 'dbre_or snapshot already running';
  END IF;

  INSERT INTO dbre_or.snapshots (
    label,
    snapshot_type,
    aurora_version,
    top_sql_limit,
  ...
  INSERT INTO dbre_or.settings_snapshot (snapshot_id, setting_name, setting_value, unit, source)
  SELECT
    v_snapshot_id,
    name,
    setting,
  ...
  INSERT INTO dbre_or.activity_snapshot (
    snapshot_id,
    state,
    wait_event_type,
    wait_event,
  ...
  INSERT INTO dbre_or.session_snapshot (
    snapshot_id,
    pid,
    usename,
    datname,
  ...
  INSERT INTO dbre_or.database_stats (
    snapshot_id,
    datid,
    datname,
    numbackends,
  ...
  IF EXISTS (
    ...
    INSERT INTO dbre_or.bgwriter_stats (
      snapshot_id,
      checkpoints_timed,
      checkpoints_req,
  ...
  INSERT INTO dbre_or.table_stats (
    snapshot_id,
    relid,
    schemaname,
    relname,
  ...
  INSERT INTO dbre_or.index_stats (
    snapshot_id,
    indexrelid,
    relid,
    schemaname,
  ...
  INTO v_has_pss_wal_bytes;

  BEGIN
    IF v_has_pss_blk_read_time AND v_has_pss_blk_write_time AND v_has_pss_wal_bytes THEN
      INSERT INTO dbre_or.statement_stats (
        snapshot_id,
        dbid,
        userid,
        queryid,
  ...
  INSERT INTO dbre_or.replication_stats (
    snapshot_id,
    application_name,
    client_addr,
    state,
  ...
  INSERT INTO dbre_or.replication_slots (
    snapshot_id,
    slot_name,
    slot_type,
    active,
