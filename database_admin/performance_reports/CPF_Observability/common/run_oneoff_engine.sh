...
collect_sqlserver_family() {
  ...
  append_section "Uptime and Connection Pressure" run_sqlserver_sql "SELECT sqlserver_start_time FROM sys.dm_os_sys_info; SELECT COUNT(*) AS current_sessions FROM sys.dm_exec_sessions;"
  append_section "Workload Counters" run_sqlserver_sql "SELECT counter_name, cntr_value FROM sys.dm_os_performance_counters WHERE counter_name IN ('Batch Requests/sec','SQL Compilations/sec','SQL Re-Compilations/sec','User Connections');"
  append_section "Top Waits" run_sqlserver_sql "SELECT TOP 25 wait_type, waiting_tasks_count, wait_time_ms, signal_wait_time_ms FROM sys.dm_os_wait_stats ORDER BY wait_time_ms DESC;"
  append_section "Top CPU Queries" run_sqlserver_sql "SELECT TOP 20 qs.execution_count, qs.total_worker_time/1000 AS total_cpu_ms, qs.total_elapsed_time/1000 AS total_elapsed_ms, qs.total_logical_reads, SUBSTRING(st.text, (qs.statement_start_offset/2)+1, ((CASE qs.statement_end_offset WHEN -1 THEN DATALENGTH(st.text) ELSE qs.statement_end_offset END - qs.statement_start_offset)/2)+1) AS sql_text FROM sys.dm_exec_query_stats qs CROSS APPLY sys.dm_exec_sql_text(qs.sql_handle) st ORDER BY qs.total_worker_time DESC;"
  append_section "Top Duration Queries" run_sqlserver_sql "SELECT TOP 20 qs.execution_count, qs.total_elapsed_time/1000 AS total_elapsed_ms, qs.total_worker_time/1000 AS total_cpu_ms, qs.total_logical_reads, LEFT(st.text, 240) AS sql_text FROM sys.dm_exec_query_stats qs CROSS APPLY sys.dm_exec_sql_text(qs.sql_handle) st ORDER BY qs.total_elapsed_time DESC;"
  append_section "Blocking Sessions" run_sqlserver_sql "SELECT TOP 25 r.session_id, r.blocking_session_id, r.wait_type, r.wait_time, r.cpu_time, r.logical_reads, LEFT(t.text,240) AS sql_text FROM sys.dm_exec_requests r OUTER APPLY sys.dm_exec_sql_text(r.sql_handle) t WHERE r.blocking_session_id <> 0 ORDER BY r.wait_time DESC;"
  append_section "Database IO by File" run_sqlserver_sql "SELECT DB_NAME(vfs.database_id) AS db_name, mf.physical_name, vfs.num_of_reads, vfs.num_of_writes, vfs.io_stall_read_ms, vfs.io_stall_write_ms FROM sys.dm_io_virtual_file_stats(NULL,NULL) vfs JOIN sys.master_files mf ON vfs.database_id = mf.database_id AND vfs.file_id = mf.file_id ORDER BY (vfs.io_stall_read_ms + vfs.io_stall_write_ms) DESC;"
  append_section "AlwaysOn / Replica Health" run_sqlserver_sql "SELECT replica_server_name, role_desc, connected_state_desc, synchronization_health_desc FROM sys.dm_hadr_availability_replica_states ars JOIN sys.availability_replicas ar ON ars.replica_id = ar.replica_id;"
}
