#!/usr/bin/env bash
# Performance baseline comparison runner for PostgreSQL migration
# Author: Nitish Anand Srivastava

set -euo pipefail

SOURCE_CONN="${SOURCE_CONN:-psql -h onprem-pg01 -p 5432 -U postgres -d salesdb -t -A}"
TARGET_CONN="${TARGET_CONN:-psql -h pg-flex-prod.postgres.database.azure.com -p 5432 -U mig_user -d salesdb -t -A}"
